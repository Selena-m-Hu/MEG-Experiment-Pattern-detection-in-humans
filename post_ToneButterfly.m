...
%     trigger_length = 0.25; % 250ms.
    ...
    SUBJECT_GLOBAL = [2:13,15];

    if dss_flag == 1
        if config.nicolas_procedure == 0
            % In this case, we use DSS data for this configuration.
            load(fullfile('..','Results',out_folder,'DSS_components','Transformed',sprintf('Xdss-TRIG_%d-%d-COMP_%d',trigger_list(1), trigger_list(2), n_components)));
        else
            load(fullfile('..','Results',out_folder,'DSS_components','Transformed',sprintf('Xdss-TRIG_%d-%d-COMP_%d_MAX',trigger_list(1), trigger_list(2), n_components)));
        end
    end
    
    for trigger_ind = 1:length(trigger_list)
        for subject_ind = 1:length(subject_list)
            subject_aux = find(subject_list(subject_ind) == SUBJECT_GLOBAL);
            % We load the timelock information that we computed in a
            % previous stage.
            
            load(fullfile('..','Results',out_folder,'Preprocessed_data_AllChannels',sprintf('Long_timelock-TRIG_%d-SUBJ_%d.mat',trigger_list(trigger_ind),subject_list(subject_ind))),'timelock')
...
%             clear timelock
%             load(fullfile('..','Results',out_folder,'Preprocessed_data',sprintf('Long_timelock-TRIG_%d-SUBJ_%d.mat',trigger_list(trigger_ind),subject(subject_ind))))
...
%             channels_num = 1:274;
            ...
            for t_ind = 1:window_size:length(timelock.time)
                ...
                if t_ind == 1
                    trigger_shape274(:,:,counter) = zeros(274,150);
