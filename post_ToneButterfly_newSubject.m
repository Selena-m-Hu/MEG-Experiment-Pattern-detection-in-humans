function post_ToneButterfly_newSubject(trigger_list, subject_list, config)
    % trigger_list: 
...
%    trigger_length = 0.25; % 250ms.
   ...
   SUBJECT_GLOBAL = [16:24];   %Subjects 2:15, aquired by Antonio %Subjects 16:24 aquired by MYH, RB

    if dss_flag == 1
       
            if single %DSS computed across conditions                 
                    load(fullfile('..','Results',out_folder,'DSS_components','Transformed',...
                    sprintf('ALLchann_allSubj_Xdss-TRIG_%d-%d-SINGLE-COMP_%d',trigger_list(1), trigger_list(2), n_components)));           
            end 
    end
...
%                 sprintf('Xdss-TRIG_%d-SUBJ_%d-SINGLE-Clean-COMP_%d',trigger_list(trigger_ind),subject_list(subject_ind), n_components))) 
...
%           channels_num = 1:274;
            ...
            for t_ind = 1:window_size:length(timelock.time)
                ...
                % particularly, subject 16 only has 272 channels, but all
                % channels have been fixed and updated as 275 for all
                % subjects
                %-----------------------------------------------------------
                if t_ind == 1
...
%            Multiply by a constant in order to get the units into
            ...
            switch baseline
                ...
                case 'tone' 
                    ...
                    baseline_data = mean(stable_average_shape(:,bl_window,subject_ind, trigger_ind), 2);
                    baseline_data274 = mean(stable_average_shape275(:,15:30,subject_ind, trigger_ind), 2);
                    output_appendix = '_tone';
                case 'silence'
                    baseline_data = mean(timelock.avg(channels_num, 1:fs*pre_estim),2)*1e15;
                    baseline_data274 = mean(timelock275.avg(:, 1:fs*pre_estim),2)*1e15;
                    output_appendix = '_silence';
                case 'activity'
                    baseline_data = mean(timelock.avg(channels_num, fs*(.5+2.0):fs*(.5+2.5)),2)*1e15;
                    baseline_data274 = mean(timelock275.avg(:, fs*(.5+2.0):fs*(.5+2.5)),2)*1e15;
           ...
           % stable_shape(:,:,:,subject_ind, trigger_ind) = trigger_shape - repmat(mean(stable_average_shape(:,1:30,subject_ind, trigger_ind) ,2), 1, 150, size(trigger_shape,3));
            ...
            stable_average_shape275(:,:,subject_ind, trigger_ind)  =  stable_average_shape275(:,:,subject_ind, trigger_ind) - repmat(baseline_data274, 1, 150);
...
%   output_appendix = 'no_baseline';
    ...
    sprintf('Tone_40Channels_newSub-SINGLE-COMP_%d_BL_%s', n_components, output_appendix)), 'stable_average_shape');

    save(fullfile('..','Results',out_folder,'tone_analysis',...
    sprintf('Tone_275Channels_newSub-SINGLE-COMP_%d_BL_%s', n_components, output_appendix)), 'stable_average_shape275');
...
%     
%     figure(1);
    plot(subject_data.time, squeeze(mean(mean_subjects_data(:,subject_list_short,:),2)*100), 'Linewidth',3)
    hold on
    ...
    title(sprintf('All new subjects. Baseline: %s', baseline));
...
%     ylim([0,12])
    ...
    subject_data.avg = mean(mean(stable_average_shape275(:,:,subject_list_short,:),4),3);
    for ind_T = 1:5
        switch ind_T
            case 1
                T_plot = [-50,0];
    ...
    if store_output == 1
        ...
        savefig(fullfile('..','Results',out_folder,'ToneTopography',sprintf('GLOBAL_%s_New_Subjects_Topography_%.2f-T%.2f_HP%d_LP%d_BLwindow_%s.fig', baseline, T_init, T_end,hpfreq,lpfreq,bl_tone)));
    end
%     
%     if store_output == 1
