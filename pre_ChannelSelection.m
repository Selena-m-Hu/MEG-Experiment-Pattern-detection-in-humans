function [channels, channels_num] = pre_ChannelSelection(trigger_list, subject, config)
    ...
    if strcmp(channel_modality,'temporal')
        ...
        % response, which needs to be analysed before this step 
        switch subject
            case 2 
                low_t       = .17;
                high_t      = .18;
...
%                 high_t = .23;
        ...
        for count=1:length(ampsSorted)
            ...
            if  subject == 6 | subject == 8 | subject == 9  |  subject == 10 | subject == 13
    ...
    if store_data == 1
        ...
        if DSS_signals == 1
            if strcmp(channel_modality,'temporal') == 1 % Temporal case  
                ...
                mkdir(fullfile('..','Results','Channels_DSS'));
                save(fullfile('..','Results','Channels_DSS',sprintf('Channels-SUBJ_%d', subject)),'channels', 'channels_num');
                savefig(fullfile('..','Results','Channels_DSS',sprintf('Topography-SUBJ_%d', subject)));
            elseif strcmp(channel_modality,'auto') == 1
                mkdir(fullfile('..','Results','Channels_DSS'));
                save(fullfile('..','Results','Channels_DSS',sprintf('Channels-auto-SUBJ_%d', subject)),'channels', 'channels_num');
            end
        else
            if strcmp(channel_modality,'temporal') == 1 % Temporal case  
