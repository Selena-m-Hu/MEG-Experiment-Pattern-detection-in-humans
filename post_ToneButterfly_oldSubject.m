function post_ToneButterfly_oldSubject(trigger_list, subject_list, config)
    % trigger_list: 
...
%    trigger_length = 0.25; % 250ms.
   ...
   SUBJECT_GLOBAL = [2:13,15];   %Subjects 2:15, aquired by Antonio %Subjects 16:24 aquired by MYH, RB
    ...
    if dss_flag == 1
            ...
            if single %DSS computed across conditions
                ...
                sprintf('Xdss-TRIG_%d-%d-SINGLE-COMP_%d',trigger_list(1), trigger_list(2), n_components)));
...
%           channels_num = 1:274;
            ...
            for t_ind = 1:window_size:length(timelock.time)
                ...
                % particularly, subject 16 only has 272 channels, so the
                % original script was modified to adapt to the new data
                %-----------------------------------------------------------
                if t_ind == 1
...
%   output_appendix = 'no_baseline';
    ...
    sprintf('Tone_40Channels_allSub-SINGLE-COMP_%d_BL_%s', n_components, output_appendix)), 'stable_average_shape');

    save(fullfile('..','Results',out_folder,'tone_analysis',...
    sprintf('Tone_273Channels_allSub-SINGLE-COMP_%d_BL_%s', n_components, output_appendix)), 'stable_average_shape274');
