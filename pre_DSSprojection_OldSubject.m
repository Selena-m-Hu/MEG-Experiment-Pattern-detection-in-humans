function pre_DSSprojection_OldSubject(trigger_list, subject_list, config)
    ...
    n_components = config.n_components;
        ...
        for subject_ind = 1:length(subject_list)
            ...
            if single == 0
                load(fullfile('..','Results_Antonio_S1_S15',out_folder,'DSS_components',...
...
%                 x_orig{trigger_ind} = cat(3,data_subject_BS.trial{:});
                x_orig{trigger_ind} = nt_trial2mat(data_subject_BS.trial); 
...
%                   script
                 ...
                 if store_output == 1
                  ...
                  %  mkdir(fullfile('..','Results',out_folder,'DSS_components','Transformed')) % data structure is saved as 3D 
                  %  mkdir(fullfile('..','Results',out_folder,'DSS_components','New_Transformed'));  %data structure has been normalised as FieldTrip expects
                    if single == 0
                        save(fullfile('..','Results',out_folder,'DSS_components','New_Transformed',...
                            sprintf('Xdss-TRIG_%d-SUBJ_%d-COMP_%d.mat',trigger_list(trigger_ind), subject_list(subject_ind), n_components)),'x_dss2');
                    else
                        save(fullfile('..','Results',out_folder,'DSS_components','New_Transformed',...
                            sprintf('Xdss-TRIG_%d-SUBJ_%d-SINGLE-COMP_%d.mat',trigger_list(trigger_ind), subject_list(subject_ind), n_components)),'x_dss2');
                    end
                 end
