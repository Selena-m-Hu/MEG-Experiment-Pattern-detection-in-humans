 function DSScomputation_Debugging(trigger_list, subject_list, config)
        ...
        for subject_ind = 1:length(subject_list)
            ...
            for trigger_ind = 1:length(trigger_list)
                ...
                if trigger_list(trigger_ind) == 5 | trigger_list(trigger_ind) == 15
                    cfg.toilim = [-0.2, 4]; % From -200ms to 4 seconds for SHORT. 4.2 seconds
                else
                    cfg.toilim = [-0.2,16]; % From -200ms to 16 seconds for LONG. 16.2 seconds
                end 
                ...
                x_orig{trigger_ind} = cat(3,data_subject_BS.trial{:}); %catenate the trial matrix in third dimension
                ...
                if store_data == 1
                    mkdir( fullfile('..','Results',out_folder,'DSS_debugging','Plots'))
                    if single == 0
                        savefig(f1, fullfile('..','Results',out_folder,'DSS_debugging','Plots',sprintf('Power-TRIG_%d_%d-SUBJ_%d', trigger_list(1),trigger_list(2), subject_list(subject_ind))));
                        savefig(f2, fullfile('..','Results',out_folder,'DSS_debugging','Plots',sprintf('Components-TRIG_%d_%d-SUBJ_%d', trigger_list(1),trigger_list(2), subject_list(subject_ind))))
                        % We store the transformed data so we can use it later.
                        mkdir(fullfile('..','Results',out_folder,'DSS_debugging'))
                        save(fullfile('..','Results',out_folder,'DSS_debugging',sprintf('DataDSS-TRIG_%d_%d-SUBJ_%d-COMP_%d.mat',trigger_list(1), trigger_list(2), subject_list(subject_ind), 274)),'z_timelock','todss', '-v7.3');
                    else                        
                        savefig(f1, fullfile('..','Results',out_folder,'DSS_debugging','Plots',sprintf('Power-TRIG_%d-SUBJ_%d', trigger_list(1), subject_list(subject_ind))));
                        savefig(f2, fullfile('..','Results',out_folder,'DSS_debugging','Plots',sprintf('Components-TRIG_%d-SUBJ_%d', trigger_list(1), subject_list(subject_ind))))
                        % We store the transformed data so we can use it later.
                        mkdir(fullfile('..','Results',out_folder,'DSS_debugging'))
                        save(fullfile('..','Results',out_folder,'DSS_debugging',sprintf('DataDSS-TRIG_%d-SUBJ_%d-COMP_%d.mat',trigger_list(1), subject_list(subject_ind), 274)),'z_timelock','todss', '-v7.3');
                    end
                end
